...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "saas-ai-terraform-state-${data.aws_caller_identity.current.account_id}"
  
  tags = {
    Name        = "Saas AI Terraform State Store"
    Environment = "global"
    ManagedBy   = "terraform"
  }
}
...
resource "aws_dynamodb_table" "terraform_locks" {
  ...
  tags = {
    Name        = "Saas AI Terraform State Locks"
    Environment = "global"
    ManagedBy   = "terraform"
  }
}
